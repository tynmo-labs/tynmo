...
func FetchECDSAValidators(
	...
	for _, addr := range valAddrs {
		accountStake, err := staking.QueryAccountStake(transition, from, addr)
		if err != nil {
			continue
		}
		if err := ecdsaValidators.Add(validators.NewECDSAValidator(addr, accountStake)); err != nil {
			return nil, err
		}
	}
...
func FetchBLSValidators(
	...
	for idx := range valAddrs {
		...
		accountStake, err := staking.QueryAccountStake(transition, from, valAddrs[idx])
		if err != nil {
			continue
		}
