...
// ParseECDSAValidator parses ECDSAValidator represented in string
func ParseECDSAValidator(validator string) *ECDSAValidator {
	return NewECDSAValidator(types.StringToAddress(validator))
}
...
func ParseBLSValidator(validator string) (*BLSValidator, error) {
	...
	return NewBLSValidator(types.BytesToAddress(addrBytes), pubKeyBytes), nil
}
