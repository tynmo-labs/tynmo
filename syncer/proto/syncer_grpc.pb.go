...
const (
	...
	SyncPeer_GetEpochSnapshot_FullMethodName = "/v1.SyncPeer/GetEpochSnapshot"
)
...
type SyncPeerClient interface {
	...
	GetEpochSnapshot(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (*EpochSnapshot, error)
}
...
func (c *syncPeerClient) GetEpochSnapshot(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (*EpochSnapshot, error) {
	out := new(EpochSnapshot)
	err := c.cc.Invoke(ctx, SyncPeer_GetEpochSnapshot_FullMethodName, in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type SyncPeerServer interface {
	...
	// Returns stream of the latest epoch snapshot
	GetEpochSnapshot(context.Context, *emptypb.Empty) (*EpochSnapshot, error)
...
func (UnimplementedSyncPeerServer) GetEpochSnapshot(context.Context, *emptypb.Empty) (*EpochSnapshot, error) {
	return nil, status.Errorf(codes.Unimplemented, "method GetEpochSnapshot not implemented")
}
...
func _SyncPeer_GetEpochSnapshot_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(emptypb.Empty)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(SyncPeerServer).GetEpochSnapshot(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: SyncPeer_GetEpochSnapshot_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(SyncPeerServer).GetEpochSnapshot(ctx, req.(*emptypb.Empty))
	}
	return interceptor(ctx, in, info, handler)
}
...
var SyncPeer_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "GetEpochSnapshot",
			Handler:    _SyncPeer_GetEpochSnapshot_Handler,
		},
	},
