...
func (p *genesisParams) validateFlags() error {
	...
	// Check that the epoch size is correct
	if p.epochSize < 2 && server.ConsensusType(p.consensusRaw) == server.TynmoBFTConsensus {
		// Epoch size must be greater than 1, so new transactions have a chance to be added to a block.
		// Otherwise, every block would be an endblock (meaning it will not have any transactions).
		// Check is placed here to avoid additional parsing if epochSize < 2
		return errInvalidEpochSize
...
func (p *genesisParams) isTYNMOBFTConsensus() bool {
	return server.ConsensusType(p.consensusRaw) == server.TynmoBFTConsensus
}
...
func (p *genesisParams) initIBFTExtraData() {
	if p.consensus != server.IBFTConsensus && p.consensus != server.TynmoBFTConsensus {
		return
	}
...
func (p *genesisParams) initConsensusEngineConfig() {
	...
	p.initIBFTEngineMap(p.consensus, fork.PoA)
}

func (p *genesisParams) initIBFTEngineMap(consensus server.ConsensusType, ibftType fork.IBFTType) {
	p.consensusEngineConfig = map[string]interface{}{
		string(consensus): map[string]interface{}{
			fork.KeyType:          ibftType,
			fork.KeyValidatorType: p.ibftValidatorType,
			tynmobft.KeyEpochSize: p.epochSize,
		},
	}
}
...
func (p *genesisParams) shouldPredeployStakingSC() bool {
	...
	return p.isPos && (p.consensus == server.TynmoBFTConsensus || p.consensus == server.IBFTConsensus || p.consensus == server.DevConsensus)
}
